...
class browser_pool:
    ...
    def run(self,fun,text):
        ...
        if data!=False:
            ...
            self.list_network.extend(fun.callback_network())
            self.list_res.append(data)
            print(data)
    def control(self,lists):
        try:
            list_browser=[webbrowser.webbrowser() for i in range(5)]
            list_text=[]
            j=0
            for i in lists:
                ...
                j=j+1
                if j>=5:
                    tsk = [threading.Thread(target=self.run,args=(list_browser[k['signal']],k['url'])) for k in list_text] 
                    [ks.start() for ks in tsk]
                    [ks.join() for ks in tsk]
                    self.write('href_tmp', list(set(self.list_href)))
                    self.write('network_tmp', list(set(self.list_network)))
                    self.list_href=[]
                    self.list_network=[]
                    j=0
                    list_text=[]
